...
module.exports = class Cml {
	...
	*/
	...
	virtualTotal(user, callback) {
		let url = this.cmlURL + '?type=virtualhiscoresatplayer&player=' + user + '&page=virtualtotal'

		request(url, (error, response, body) => {
			let errorValue = errorHandling(body)
			body = body.split(',')
			let rank = parseInt(body[0])
			let level = parseInt(body[2])

			callback(errorValue, level, rank)
		})
	}

	recordsHeld(user, callback) {
		let url = this.cmlURL + '?type=virtualhiscoresatplayer&player=' + user + '&page=recordsheld'

		request(url, (error, response, body) => {
			let errorValue = errorHandling(body)
			body = body.split(',')
			let rank = parseInt(body[0])
			let records = parseInt(body[2])

			callback(errorValue, records, rank)
		})
	}

	frontpageCount(user, callback) {
		let url = this.cmlURL + '?type=virtualhiscoresatplayer&player=' + user + '&page=frontpagecount'

		request(url, (error, response, body) => {
			let errorValue = errorHandling(body)
			body = body.split(',')
			let rank = parseInt(body[0])
			let count = parseInt(body[2])

			callback(errorValue, count, rank)
		})
	}
