...
module.exports = class Cml {
	...
	stats(user, callback) {
		...
		let stats = {}
		let errorValue = []
		let requests = 2 // number requests that has been made
		let requestsDone = 0 // number of requests that has been done
		...
		request(url, (error, response, body) => { // need to have an if error thingy
			errorValue.push(errorHandling(body))
